...
def parse_args():
    parser = argparse.ArgumentParser(description='Aggregate data by ID and compute mean a specified column.')
...
def process_data(df, column_name):
    new_df = df[['id', column_name]].groupby('id').mean().reset_index()
    return new_df
